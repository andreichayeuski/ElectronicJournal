...
{
	internal class CoreDropDownListRenderer
	{
		private CoreDropDownList _model;

		public CoreDropDownListRenderer(CoreDropDownList model)
		{
			_model = model;
		}
		...
		public string RenderHtml()
		{
			...
			//}
			Guard.IsNotNullOrEmpty(_model.ID, "ID", "You need to set ID for this CoreDropDownList instance.");
		...
		private string GetStandaloneJavascript()
		{
			...
			stringBuilder.AppendFormat("jQuery('#{0}').jqDropDownList({{", _model.ID);
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			CoreDropDownList model = _model;
			DropDownListClientSideEvents clientSideEvents = _model.ClientSideEvents;
