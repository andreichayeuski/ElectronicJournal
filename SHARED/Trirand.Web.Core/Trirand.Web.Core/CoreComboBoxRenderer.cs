...
{
	internal class CoreComboBoxRenderer
	{
		private CoreComboBox _model;

		public CoreComboBoxRenderer(CoreComboBox model)
		{
			_model = model;
		}
		...
		private string GetStandaloneJavascript()
		{
			...
			stringBuilder.AppendFormat("jQuery('#{0}').jqComboBox({{", _model.ID);
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			CoreComboBox model = _model;
			ComboBoxClientSideEvents clientSideEvents = _model.ClientSideEvents;
